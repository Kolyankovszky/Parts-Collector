...
if (isset($_POST['submit'])){
	...
	if (empty($pumpmarka) || empty($pumpmodell) || empty($pumpsorozat) || empty($pumpgyariszam) || empty($ugyfelid)){
		...
		$sql = "SELECT pumpgyariszam FROM szivattyuk WHERE pumpgyariszam = ?";
		$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)) {
			header("Location: index.php?error=sqlerror");
			exit();
		} else {
			mysqli_stmt_bind_param($stmt, "s", $pumpgyariszam);
			mysqli_stmt_execute($stmt);
			mysqli_stmt_store_result($stmt);
			$rowCount = mysqli_stmt_num_rows($stmt);
			
			if ($rowCount > 0) {
				header("Location: index.php?error=pumpgyariszamtaken");
				exit();
			} else {
